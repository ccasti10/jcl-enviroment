...
function openConfigWebView(extensionUri, initialConfig, onSave) {
    const panel = vscode.window.createWebviewPanel('jclSwitcherConfig', 'JCL Switcher: Configurar Ambientes', vscode.ViewColumn.One, {
        enableScripts: true,
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        ...
        if (message.type === 'save') {
            try {
                ...
                vscode.window.showInformationMessage(`JCL Switcher: configuración guardada en ${savedPath}.`);
            }
            catch (error) {
                ...
                vscode.window.showErrorMessage(`JCL Switcher: no se pudo guardar la configuración. ${messageText}`);
            }
        }
    });
}
function getHtml(webview, extensionUri) {
    const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'configWebView.css'));
    const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'configWebView.js'));
    return /* html */ `
<!DOCTYPE html>
<html lang="es">
<head>
 ...
 */
function searchBar(prefix, label, placeholder) {
    return /* html */ `
