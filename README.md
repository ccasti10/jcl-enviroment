...
Extensi�n de VS Code para cambiar las referencias para los distintos ambientes de trabajo�DESARROLLO, TEST,
PRODUCCION�dentro de un JCL de mainframe, sin buscar y reemplazar a mano.

Abre un JCL, elige el ambiente destino y la extensi�n reescribe los `DSN=`, los
bloques `JOBLIB`/`STEPLIB` y los par�metros como `CLASS=` entre otros, respetando las columnas
...
Al hacer clic se elige el ambiente destino. Si el archivo mezcla se�ales de varios
...
la pesta�a **Bloques**, linea por linea, y se copia tal cual.
